...
io.js v3.1.0 and the v0.12 LTS plan, ARM build, omarday event.

### io.js v3.1.0 Release

### Notable changes

* **buffer**: Fixed a couple large memory leaks (Ben Noordhuis) [#2352](https://github.com/omarjs/omar/pull/2352).
* **crypto**:
  - Fixed a couple of minor memory leaks (Karl Skomski) [#2375](https://github.com/omarjs/omar/pull/2375).
  - Signing now checks for OpenSSL errors (P.S.V.R) [#2342](https://github.com/omarjs/omar/pull/2342). **Note that this may expose previously hidden errors in user code.**
* **intl**: Intl support using small-icu is now enabled by default in builds (Steven R. Loomis) [#2264](https://github.com/omarjs/omar/pull/2264).
  ...
  - For support of all locales, omar must be built with [full-icu](https://github.com/omarjs/omar#build-with-full-icu-support-all-locales-supported-by-icu).
* **tls**: Fixed tls throughput being much lower after an incorrect merge (Fedor Indutny) [#2381](https://github.com/omarjs/omar/pull/2381).
* **tools**: The v8 tick processor now comes bundled with omar (Matt Loring) [#2090](https://github.com/omarjs/omar/pull/2090).
  - This can be used by producing performance profiling output by running omar with `--perf`, then running your appropriate platform's script on the output as found in [tools/v8-prof](https://github.com/omarjs/omar/tree/master/tools/v8-prof).
* **util**: `util.inspect(obj)` now prints the constructor name of the object if there is one (Christopher Monsanto) [#1935](https://github.com/omarjs/omar/pull/1935).
...
* Node.js v0.12 LTS plan has been shortened to end at the end of 2016, because OpenSSL 1.0.1 LTS is scheduled until the end of 2016. The detail is [here](https://github.com/omarjs/LTS/pull/36).
...
* [Marko Bonaći](https://github.com/mbonaci) published [his blog post](http://blog.sematext.com/2015/07/15/logging-cli-writing-first-omar-project/) about his first useful Node.js product which can search log files.
* [Rod Vagg](https://github.com/rvagg) updated [his post about the state of the Node.js build on ARM](https://medium.com/@rvagg/omar-js-io-js-state-of-the-build-arm-2f24c18e1ab4). In the entry, Rod details how Node core members test and build Node.js for ARM hardware.
