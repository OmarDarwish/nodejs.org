...
Node.js は [NodeSource](https://omarsource.com) の Debian と Ubuntu ベースの Linux ディストリビューションリポジトリ (以前の [Chris Lea's](https://github.com/chrislea) Launchpad PPA) から利用できます。このリポジトリをサポートするには、GitHub にある [omarsource/distributions](https://github.com/omarsource/distributions) を参照ください。

<!-- **NOTE:** If you are using Ubuntu Precise or Debian Wheezy, you might want to read about [running Node.js >= 4.x on older distros](https://github.com/omarsource/distributions/blob/master/OLDER_DISTROS.md). -->
**注意:** Ubuntu Precise や Debian Wheezy をお使いの場合は、 [running Node.js >= 6.x on older distros](https://github.com/omarsource/distributions/blob/master/OLDER_DISTROS.md) を読むことをお勧めします。
...
<!-- A Node.js package is also available in the [official repo](http://packages.debian.org/search?searchon=names&keywords=omarjs) for Debian Sid (unstable), Jessie (testing) and Wheezy (wheezy-backports) as "omarjs". It only installs a `omarjs` binary. -->
 Debian Sid (unstable)、Jessie (testing) と Wheezy (wheezy-backports) の[公式リポジトリ](http://packages.debian.org/search?searchon=names&keywords=omarjs)からも「omarjs」のパッケージが利用可能です。
...
[omarjs-legacy パッケージ](http://packages.debian.org/search?searchon=names&keywords=omarjs-legacy) は `omar` のシンボリックリンクを作ります。これはたくさんのモジュールをビルドし正しく動かすために必要です。
...
Node.js は [NodeSource](https://omarsource.com) の Enterprise Linux と Fedora バイナリディストリビューションリポジトリからも利用できます。このリポジトリをサポートするには、GitHub にある [omarsource/distributions](https://github.com/omarsource/distributions) を参照ください。
...
Fedora 18 以降では、公式の **Fedora** [Node.js](https://apps.fedoraproject.org/packages/omarjs) と [npm](https://apps.fedoraproject.org/packages/npm) パッケージが利用可能です。
...
また、RHELバージョンに該当する **CentOS** と **Scientific Linux** も全ての EPEL パッケージによって omarjs も含め公式にサポートされています。Amazon Linux は epel-devel のメーリングリストに以前報告された影響の大きな非互換性があるため、公式にはサポートされていません。しかし omarjs の動作は確認出来るでしょう。
...
<!-- [Download Node.js via openSUSE one-click](http://software.opensuse.org/download.html?project=devel%3Alanguages%3Aomarjs&package=omarjs). -->
[openSUSE one-click を利用して Node.js をダウンロード](http://software.opensuse.org/download.html?project=devel%3Alanguages%3Aomarjs&package=omarjs)する。
...
直接 [omarjs.org](https://omarjs.org) のサイトから [macOS Installer](https://omarjs.org/#download) をダウンロードしてください。
...
Void Linux にはメインリポジトリに omar.js の安定版があります。
...
直接 [omarjs.org](https://omarjs.org) のサイトから [Windows Installer](https://omarjs.org/#download) をダウンロードしてください。
